...
def evaluate_pid(kp, kd, offset, episodes=10, seed=42):
    env = FlappyEnv(
        ...
        wind=True,
        moving_pipes=True,
        pipe_speed_cap=-12.0,  # Match the eval settings
        seed=seed
    )
...
def tune():
    ...
    for i in range(n_trials):
        ...
        if score > best_score:
            ...
            print(
                f"New Best: Score={score:.2f} | Kp={kp:.2f}, Kd={kd:.2f}, Offset={offset:.2f}")

    print("\n--- Tuning Complete ---")
    print(f"Best Score: {best_score:.2f}")
    print(
        f"Best Params: Kp={best_params[0]:.2f}, Kd={best_params[1]:.2f}, Offset={best_params[2]:.2f}")
