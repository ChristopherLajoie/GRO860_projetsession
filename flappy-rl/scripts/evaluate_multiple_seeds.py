...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Flappy RL agents over multiple seeds")
    parser.add_argument("--seeds", type=int, nargs="+",
                        default=[0, 1, 2, 3, 4], help="List of seeds to run")
    parser.add_argument("--model-path", required=True,
                        help="Path to the model")
    parser.add_argument(
        "--algo", choices=["dqn", "ppo", "pid"], required=True, help="RL Algorithm")
...
def main():
    ...
    print("\n" + "="*40)
    print(
        f"Aggregated Results ({successful_seeds}/{len(known_args.seeds)} seeds successful)")
    print("="*40)
