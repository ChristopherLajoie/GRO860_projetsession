...
def get_obs(bird_y, bird_vy, dx, gap_center_y, gap_height, pipe_vx, wind):
    ...
    # Wind=True
    features = [norm_bird_y, norm_bird_vy, norm_dx,
                norm_gap_center, norm_gap_height, norm_pipe_vx, norm_wind]
...
def plot_policy(model_path, output_file, wind_value=0.0):
    ...
    for i, vy in enumerate(vys):
        for j, y in enumerate(ys):
            ...
            obs_tensor = torch.as_tensor(
                obs_stacked).unsqueeze(0).to(model.device)
    ...
    plt.imshow(probs, extent=[0, HEIGHT, -MAX_VY, MAX_VY],
               origin='lower', aspect='auto', cmap='coolwarm', vmin=0, vmax=1)
